...
class DeepLoco(nn.Module):
    def __init__(self, net_filters = 16, conv_block_depth = 2, initial_filter_size = 5, filter_size = 3, resnet_out_dim = 1024, resnet_depth = 2, max_points = 5, min_coords = [0,0,0], max_coords = [64,64,0] ):
...
class WeightsAndLocations(nn.Module):
    def __init__(self, feature_dim, MAX_SOURCES, source_dim, min_coord, max_coord):
        ...
        self.locations = nn.Linear(feature_dim, 64*64)
    ...
    def forward(self, input):
        ...
        thetas = F.sigmoid(self.locations(input))
        # if thetas.is_cuda:
